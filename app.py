...
REQUEST_LIMIT = 6000  # The token limit per minute
REQUEST_INTERVAL = 60  # Time interval in seconds
last_request_time = 0  # Time of the last request

def rate_limited_request(query, conversation):
    ...
    if time_since_last_request < REQUEST_INTERVAL:
        ...
        st.warning(f"Rate limit reached. Waiting for {wait_time:.2f} seconds...")
...
# Initialize the conversation memory with a window length of 5
...
if user_input:
    ...
    with st.spinner("Thinking..."):
        bot_response = rate_limited_request(user_input, rag_chain)
